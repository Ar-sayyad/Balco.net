...
$.each(cpp600DGR, function (key) {
    var batch = {
        "database": {
            "Method": "GET",
            "Resource": baseServiceUrl + "attributes?path=" + cpp600DGR[key].tag_path + "|" + cpp600DGR[key].parameter
        },
        "values": {
            "Method": "GET",
            "RequestTemplate": {
    ...
    $.when(batchResult).done(function () {
        ...
        $('#' + cpp600DGR[key].unitname + ' tbody').append("<tr><td>" + cpp600DGR[key].sr + "</td><td style='text-align:left;padding-left:20px;'>" + cpp600DGR[key].title + "</td><td>" + uom + "</td><td><input type='text' id='value" + cpp600DGR[key].sr + "' data-id='" + cpp600DGR[key].sr + "' data-WebId='" + WebId + "' value='" + attrValue + "' class='form-control input-manual WebId'></td><td><div class='status" + cpp600DGR[key].sr + "'></div></td></tr>")
        sort_table("tbody" + cpp600DGR[key].unitname)
    })
});
