...
{
    "title": "KPIS Plant Performance",
    "path": "path={0}|Unit1&path={0}|Unit2&path={0}|Unit3&path={0}|Unit4",
    "table": "kpispp",
    "digits": [{
        "name": "1. Boiler Efficiency",
        "decimal": 2
    },
    {
        "name": "2. STG Gross Heatrate",
        "decimal": 2
    },
    {
        "name": "3. Trippings (MTD)",
        "decimal": 2
    },
    {
        "name": "4. Trippings (YTD)",
        "decimal": 2
    }
    ]
}
...
$.each(OverallStationAnalysis, function (key) {
    ...
    var batch1 = {
        "database": {
            "Method": "GET",
            "Resource": baseServiceUrl + "elements?path=\\\\" + afServerName + "\\" + afDatabaseName + "\\KPIs\\Summary\\PP1200 Plant Summary\\" + OverallStationAnalysis[key].title + "&selectedFields=WebId;Name;Path;Links.Elements"
        },
        "elements": {
            "Method": "GET",
    ...
    $.when(batchResult1).done(function () {
        ...
        $.each(batchResultItems, function (elementID) {
            ...
            attrItems.forEach(function (attr, attrID) {
                ...
                } else if (attrName1 === 'Unit2') {
                    elementItems1.push({
                        'Unit2': attrValue
                    });
                } else if (attrName1 === 'Unit3') {
                    elementItems1.push({
                        'Unit3': attrValue
                    });
                } else if (attrName1 === 'Unit4') {
                    elementItems1.push({
                        'Unit4': attrValue
                    });
                }
        ...
        $.each(rankingElements1, function (key) {
            var rows2 = [];
            //if (rankingElements1[key][2].Unit1 || rankingElements1[key][2].Unit1 == 0) {
            rows2.push(rankingElements1[key][0].mw, rankingElements1[key][1].UOM, rankingElements1[key][2].Unit1, rankingElements1[key][3].Unit2, rankingElements1[key][4].Unit3, rankingElements1[key][5].Unit4);
            //}
    ...
    $.each(plantSummaryPerformance, function (key) {
        ...
        if (summaryPerformance[key1].type === 'gauge') {
            var batch = {
                "database": {
                    "Method": "GET",
                    "Resource": baseServiceUrl + "elements?path=\\\\" + afServerName + "\\" + afDatabaseName + "\\KPIs\\Summary\\PP1200 Plant Summary\\Charts\\" + summaryPerformance[key1].afname + "\\" + plantSummaryPerformance[key].afname + "&selectedFields=WebId;Links.Elements"
                },
                "elements": {
                    "Method": "GET",
            ...
            var batch3 = {
                "database": {
                    "Method": "GET",
                    "Resource": baseServiceUrl + "elements?path=\\\\" + afServerName + "\\" + afDatabaseName + "\\KPIs\\Summary\\PP1200 Plant Summary\\Charts\\" + summaryPerformance[key1].afname + "\\" + plantSummaryPerformance[key].afname + "&selectedFields=WebId;Name;Path;Links.Attributes"
                },
                "attributes": {
                    "Method": "GET",
                    "RequestTemplate": {
